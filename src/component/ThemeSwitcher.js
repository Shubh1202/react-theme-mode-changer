...
const modeClass = {
    hover: 'hover:bg-gray-300 dark:hover:bg-gray-700 focus:ring-2 focus:ring-gray-500 dark:focus:ring-white',
    active: 'bg-white dark:bg-gray-700 text-black dark:text-white ring-2 ring-gray-500 dark:ring-gray-300'
}

const ThemeSwitcher = () => {

    const {themeMode, setLightMode, setDarkMode} = useThemeMode()

    useEffect(() => {
        const htmlNode = document.querySelector('html')
        htmlNode.classList.remove('dark')
        htmlNode.classList.add(themeMode)
        const bodyColor = (themeMode==='light') ? '#111827' : '#fff'
        ...
        <div className="row-start-5 grid lg:col-start-3">
            <div>
                <div className="mx-auto flex w-full flex-col items-start gap-6 sm:flex-row sm:items-center sm:justify-between sm:gap-8 max-w-2xl lg:max-w-5xl">
                    ...
                    >
                        <span
                            className={`${"cursor-pointer rounded-full p-2 transition-all duration-300"} ${(themeMode==='dark') ? modeClass.active : modeClass.hover}`}
                            aria-label="Light theme"
                            role="radio"
                            onClick={(e) => setDarkMode('dark')}
                        ...
                        <span
                            ...
                            className={`${"cursor-pointer rounded-full p-2 transition-all duration-300"} ${(themeMode==='light') ? modeClass.active : modeClass.hover}`}
